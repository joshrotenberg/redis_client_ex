defmodule Redis.Resilience.Retry do
  @moduledoc """
    ...
    case fun.() do
      {:error, error} = result ->
        if attempt_num < state.max_attempts and state.retry_on.(error) do
          ...
          Logger.debug("Redis.Retry: attempt #{attempt_num} failed, retrying in #{delay}ms")
          Process.sleep(delay)
  ...
  defp default_retry_on(%Redis.ConnectionError{}), do: true
