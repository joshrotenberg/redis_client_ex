defmodule Redis.Resilience.CircuitBreaker do
  @moduledoc """
  ...
  ## Options
    ...
    * `:conn` - underlying Redis.Connection (required)
  ...
  def handle_info(:half_open, state) do
    Logger.debug("Redis.CircuitBreaker: transitioning to half_open")
  ...
  defp execute(state, fun) do
    case fun.() do
      ...
      {:error, %Redis.ConnectionError{}} = result ->
  ...
  defp record_success(%{state: :half_open} = state) do
    ...
    if new_count >= state.success_threshold do
      Logger.debug("Redis.CircuitBreaker: closing circuit")
  ...
  defp record_failure(%{state: :closed} = state) do
    ...
    if new_count >= state.failure_threshold do
      Logger.warning("Redis.CircuitBreaker: opening circuit after #{new_count} failures")
      trip_open(state)
    else
  ...
  defp record_failure(%{state: :half_open} = state) do
    Logger.debug("Redis.CircuitBreaker: half_open probe failed, re-opening")
    trip_open(state)
  end
