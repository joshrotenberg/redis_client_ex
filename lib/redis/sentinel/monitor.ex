defmodule Redis.Sentinel.Monitor do
  @moduledoc """
  ...
  def handle_info(:connect, state) do
    case connect_and_subscribe(state) do
      ...
      {:error, reason} ->
        Logger.warning("Redis.Sentinel.Monitor: connect failed: #{inspect(reason)}")
  ...
  def handle_info({:tcp_closed, _socket}, state) do
    Logger.warning("Redis.Sentinel.Monitor: connection closed, reconnecting...")
    Process.send_after(self(), :connect, state.backoff_current)
  ...
  defp handle_message(["message", "+switch-master", payload], state) do
    ...
    case String.split(payload, " ") do
      ...
      _ ->
        Logger.warning("Redis.Sentinel.Monitor: unexpected +switch-master payload: #{payload}")
    end
