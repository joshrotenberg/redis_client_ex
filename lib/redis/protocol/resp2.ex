defmodule Redis.Protocol.RESP2 do
  @moduledoc """
  ...
  defdelegate encode(args), to: Redis.Protocol.RESP3
  defdelegate encode_pipeline(commands), to: Redis.Protocol.RESP3
    ...
    case :binary.split(data, @crlf) do
      [value, rest] ->
        decoded =
          case type do
            :string -> value
            :error -> %Redis.Error{message: value}
