defmodule RedisEx.PubSub do
  ...
  ## Usage
      ...
      {:ok, ps} = RedisEx.PubSub.start_link(port: 6379)
      :ok = RedisEx.PubSub.subscribe(ps, "mychannel", self())
      ...
      :ok = RedisEx.PubSub.unsubscribe(ps, "mychannel", self())
  ...
  def handle_info({:tcp_closed, _socket}, state) do
    Logger.warning("RedisEx.PubSub: connection closed")
    {:noreply, schedule_reconnect(%{state | socket: nil, state: :disconnected, buffer: <<>>})}
  end

  def handle_info({:tcp_error, _socket, reason}, state) do
    Logger.warning("RedisEx.PubSub: connection error: #{inspect(reason)}")
    {:noreply, schedule_reconnect(%{state | socket: nil, state: :disconnected, buffer: <<>>})}
  end
  ...
  def handle_info(:connect, state) do
    case connect(state) do
      ...
      {:error, reason} ->
        Logger.warning("RedisEx.PubSub: reconnect failed: #{inspect(reason)}")
  ...
  defp connect(state) do
    ...
    with {:ok, socket} <- :gen_tcp.connect(host, state.port, tcp_opts, state.timeout),
      ...
      Logger.debug("RedisEx.PubSub: connected to #{state.host}:#{state.port}")
