defmodule RedisEx.Protocol.RESP3 do
  @moduledoc """
  ...
  Returns iodata for efficient socket writing.
      ...
      iex> RedisEx.Protocol.RESP3.encode(["SET", "key", "value"]) |> IO.iodata_to_binary()
      "*3\\r\\n$3\\r\\nSET\\r\\n$3\\r\\nkey\\r\\n$5\\r\\nvalue\\r\\n"
  """
  ...
  defp decode_simple_error(data) do
    case :binary.split(data, @crlf) do
      [msg, rest] -> {:ok, %RedisEx.Error{message: msg}, rest}
      [_] -> {:continuation, &decode/1}
    end
  end
  ...
  defp decode_blob_error(data) do
    case :binary.split(data, @crlf) do
      [len_str, rest] ->
        ...
        case rest do
          <<blob::binary-size(len), "\r\n", rest2::binary>> ->
            {:ok, %RedisEx.Error{message: blob}, rest2}
