defmodule RedisEx.Cluster do
  ...
  ## Usage
      ...
      {:ok, "OK"} = RedisEx.Cluster.command(cluster, ["SET", "mykey", "myvalue"])
      {:ok, "myvalue"} = RedisEx.Cluster.command(cluster, ["GET", "mykey"])
  ...
  def handle_info({:EXIT, pid, reason}, state) do
    ...
    if reason != :normal do
      Logger.warning("RedisEx.Cluster: node connection #{inspect(pid)} died: #{inspect(reason)}")
    end
        ...
        {:ok, conn} ->
          case Connection.command(conn, ["CLUSTER", "SLOTS"]) do
            {:ok, slots_data} ->
              ...
              Logger.debug("RedisEx.Cluster: discovered #{length(nodes)} nodes, #{length(slot_entries)} slots")
    ...
    case get_connection_for_slot(state, slot) do
      {:ok, conn} ->
        case execute_on(conn, args, type) do
          {:error, %RedisEx.Error{message: "MOVED " <> rest}} ->
            # MOVED <slot> <host>:<port> — refresh topology and retry
            Logger.debug("RedisEx.Cluster: MOVED redirect: #{rest}")
          ...
          {:error, %RedisEx.Error{message: "ASK " <> rest}} ->
            # ASK <slot> <host>:<port> — one-shot redirect with ASKING
            Logger.debug("RedisEx.Cluster: ASK redirect: #{rest}")
            {_new_slot, host, port} = parse_redirect(rest)
