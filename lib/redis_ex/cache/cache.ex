defmodule RedisEx.Cache do
  @moduledoc """
  Client-side caching using RESP3 server-assisted invalidation.

  Wraps a `RedisEx.Connection` and caches read command results in an ETS table.
  ...
  ## Usage
      ...
      {:ok, cache} = RedisEx.Cache.start_link(port: 6379)

      # First call: cache miss → hits Redis
      {:ok, "bar"} = RedisEx.Cache.get(cache, "foo")
      ...
      {:ok, "newval"} = RedisEx.Cache.get(cache, "foo")

      # Stats
      RedisEx.Cache.stats(cache)
