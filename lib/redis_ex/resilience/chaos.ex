defmodule RedisEx.Resilience.Chaos do
  @moduledoc """
  ...
  ## Usage
      ...
      {:ok, conn} = RedisEx.Connection.start_link(port: 6379)
      {:ok, chaos} = RedisEx.Resilience.Chaos.start_link(
        conn: conn,
      ...
      RedisEx.Resilience.Chaos.command(chaos, ["GET", "key"])
  ...
  def init(opts) do
    ...
    state = %__MODULE__{
      ...
      error_fn: Keyword.get(opts, :error_fn, fn ->
        {:error, %RedisEx.ConnectionError{reason: :chaos_injected}}
      end)
    }
