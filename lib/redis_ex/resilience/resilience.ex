defmodule RedisEx.Resilience do
  @moduledoc """
  ...
  ## Usage
      ...
      {:ok, r} = RedisEx.Resilience.start_link(
        port: 6379,
      ...
      RedisEx.Resilience.command(r, ["GET", "key"])
      RedisEx.Resilience.pipeline(r, [["GET", "a"], ["GET", "b"]])

      # Inspect the stack
      RedisEx.Resilience.info(r)
  ...
  ## Options
    ...
    * All `RedisEx.Connection` options (host, port, password, etc.)
    * `:retry` - keyword opts for `RedisEx.Resilience.Retry`, or `false`
    * `:circuit_breaker` - keyword opts for `RedisEx.Resilience.CircuitBreaker`, or `false`
    * `:coalesce` - `true` or keyword opts for `RedisEx.Resilience.Coalesce`, or `false`
    * `:bulkhead` - keyword opts for `RedisEx.Resilience.Bulkhead`, or `false`
  """
