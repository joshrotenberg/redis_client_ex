defmodule RedisEx.Connection do
  ...
  alias RedisEx.Protocol.{RESP2, RESP3}
  ...
  Returns a child spec for supervision trees.
      ...
      children = [
        {RedisEx.Connection, port: 6379, name: :redis}
      ]
  """
  ...
  def start_link(uri) when is_binary(uri) do
    start_link(RedisEx.URI.parse(uri))
  end
  ...
  def handle_call({:command, args}, from, %{state: :ready} = state) do
    ...
    case send_data(state, data) do
      ...
      {:error, reason} ->
        {:reply, {:error, %RedisEx.ConnectionError{reason: reason}}, handle_disconnect(state)}
    end
  end
  ...
  def handle_info({closed, _socket}, state) when closed in [:tcp_closed, :ssl_closed] do
    Logger.warning("RedisEx: connection closed")
  ...
  def handle_info({error, _socket, reason}, state) when error in [:tcp_error, :ssl_error] do
    Logger.warning("RedisEx: connection error: #{inspect(reason)}")
  ...
  def handle_info(:connect, state) do
    case connect(state) do
      ...
      {:error, reason} ->
        Logger.warning("RedisEx: connect failed: #{inspect(reason)}, retrying...")
  ...
  defp connect(%{unix_socket: path} = state) when is_binary(path) do
    ...
    with {:ok, socket} <- :gen_tcp.connect({:local, String.to_charlist(path)}, 0, tcp_opts, state.timeout),
      ...
      Logger.debug("RedisEx: connected to #{path}")
  ...
  defp connect(state) do
    ...
    with {:ok, socket} <- :gen_tcp.connect(host, state.port, tcp_opts, state.timeout),
         ...
         {:ok, state} <- handshake(%{state | socket: socket, state: :ready, buffer: <<>>}) do
      set_active(state, true)
      Logger.debug("RedisEx: connected to #{state.host}:#{state.port}")
      {:ok, %{state | backoff_current: state.backoff_initial}}
    else
      {:error, reason} ->
        {:error, reason}
  ...
  defp negotiate_protocol(%{protocol: :resp3} = state) do
    ...
    case sync_command(state, args) do
      ...
      {:ok, %RedisEx.Error{message: msg}, _state} ->
        cond do
          ...
          true ->
            Logger.debug("RedisEx: HELLO 3 not supported, falling back to RESP2")
  ...
  defp maybe_auth(state) do
    ...
    case sync_command(state, args) do
      ...
      {:ok, %RedisEx.Error{message: msg}, _state} -> {:error, {:auth_failed, msg}}
  ...
  defp maybe_select(state) do
    case sync_command(state, ["SELECT", to_string(state.database)]) do
      {:ok, "OK", state} -> {:ok, state}
      {:ok, %RedisEx.Error{message: msg}, _state} -> {:error, {:select_failed, msg}}
  ...
  defp wrap_response(%RedisEx.Error{} = error), do: {:error, error}
  ...
  defp fail_pending_callers(state, reason) do
    error = {:error, %RedisEx.ConnectionError{reason: reason}}
